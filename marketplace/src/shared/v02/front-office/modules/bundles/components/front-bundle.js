...
import CarouselCmpt from 'v02/front-office/generic-components/carousel';
import CarouselItem from 'react-bootstrap/lib/CarouselItem';
import Modal from 'react-bootstrap/lib/Modal';
import HeaderCmpt from './_common/header';
import ActionColumnCmpt from './_common/actions-column';
import ProductCmpt from 'v02/front-office/modules/products/components/product-without-navigation';

const connector = connect(state => ({
  tenant: state.get('v02').get('common').get('tenant').get('name'),
  bundle: state.get('v02').get('frontOffice').get('bundles').get('current'),
...
class Bundle extends Components {
  static getActions(tenant, params = {}, query = {}) {
    ...
    return [
      actions.FrontOffice.Bundles.fetchOne(Bundle.getId(params)),
    ];
  }
  constructor(props) {
    super(props);
  ...
  onSelectProduct = product => {
    ...
    this.props.dispatch(actions.FrontOffice.Products.fetchOne(product.id));
    this.setState({
