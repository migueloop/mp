...
import SingleUserSelectorCmpt from 'v02/back-office/modules/assignments/components/single-user-selector';
...
class Assignment extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      assignmentOptions: [],
      assignmentOptionsForms: [],
    };
  }

  getAssignmentOptionsForms = () => {
    const bundleId = this.state.assignment.items[0].id_bundle;
    const productId = this.state.assignment.items[0].id_product;
    if (!bundleId) {
      const product = this.props.products.toJS().find(product => product.id === productId);
      return [
        {
          id: product.id_assignment_option_form,
          productId,
          productName: product.name,
        },
      ].filter(item => item.id !== null);
    }
    const bundle = this.props.bundles.toJS().find(bundle => bundle.id === bundleId);
    const bundleComponentIds = bundle.components.map(component => component.id);
    const products = this.props.products.toJS().filter(product => bundleComponentIds.indexOf(product.id) !== -1);
    const productAssignmentOrderFormIds = products.map(product => {
      return {
        id: product.id_assignment_option_form,
    ...
    return productAssignmentOrderFormIds.filter(item => item.id !== null);
  }
  ...
  onSubmitForm = e => {
    ...
    const assignment = Object.assign({}, this.state.assignment, { assignmentOrderOptions: this.state.assignmentOptions });
    return this.createAssignment(assignment);
  }

  onSubmitAssignmentOptionsForm = e => {
    e.preventDefault();
    const assignmentOptionsForms = this.state.assignmentOptionsForms.concat([]);
    const currentAssignmentOptionData = assignmentOptionsForms.shift();
    const assignmentOrderOptions = formSerialize(e.target);
    const dataToAdd = { productId: currentAssignmentOptionData.productId, options: assignmentOrderOptions };
    const assignmentOptions = this.state.assignmentOptions.concat([dataToAdd]);
    this.setState({ assignmentOptions, assignmentOptionsForms }, () => {
      if (assignmentOptionsForms.length === 0) {
  ...
  onClickAssignButton = e => {
    ...
    const assignmentOptionsForms = this.getAssignmentOptionsForms();
    const canAddOptions = assignmentOptionsForms.length > 0;
    if (canAddOptions) {
      return this.setState({ assignmentOptionsForms }, () => this.openAssignmentModal());
    }
  ...
  render() {
    ...
    const currentAssignmentOptionsForm = this.state.assignmentOptionsForms[0];
    const currentAssignmentOptionsFormId = currentAssignmentOptionsForm ? currentAssignmentOptionsForm.id : null;
    const AssignmentOptionsForm = getAssignmentOptionsForm(this.props.tenant, currentAssignmentOptionsFormId);
      ...
      <div className="container mp-assignments">
        ...
        { !!currentAssignmentOptionsForm &&
          <Modal show={this.state.showAssignmentModal} onHide={this.closeAssignmentOrderModal} className="assignment-order-modal" bsSize="m" animation={true}>
            <Modal.Header closeButton><FormattedMessage id="assignment_order_options" /> - { currentAssignmentOptionsForm.productName }</Modal.Header>
            <Modal.Body><div className="centered"><AssignmentOptionsForm onSubmit={this.onSubmitAssignmentOptionsForm} /></div></Modal.Body>
          </Modal>
        }
      </div>
    );
...
// <AssignmentOptionsForm onSubmit={this.onSubmitForm} />
