...
class PortabilityDiagnosticForm extends Components {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      assignmentId: props.followUp.get('assignmentOrder').get('idPoSystem'),
    };
  }
  ...
  render() {
    ...
    const targetOperatorOptions = [
      { value: 'vodafone', label: this.format({ id: 'vodafone' }) },
      { value: 'orange', label: this.format({ id: 'orange' }) },
      { value: 'verizon', label: this.format({ id: 'verizon' }) },
    ];
    ...
    return (
      <form id="fleet-updated-line-1" onSubmit={this.onSubmit} className="center-block" style={{ width: '300px' }}>
        ...
        <div className="form-group">
          ...
          <SimpleSelect disabled={true} defaultValue={defaultValue} options={targetOperatorOptions} placeholder={this.format({ id: 'select...' })} onValueChange={this.onChangeTargetOperator} />
        </div>
        <div className="form-group">
          <label><FormattedMessage id="portability_date" /></label>
          <Input disabled={true} className="form-control" type="text" id="portability_date" name="date" value={this.state.portability_date} onChange={e => this.setState({ portability_date: e.target.value })} />
        </div>
        <div className="form-group">
          <label><FormattedMessage id="account_number" /></label>
          <Input disabled={true} className="form-control" type="text" id="account_number" name="account_number" value={this.state.account_number} onChange={e => this.setState({ account_number: e.target.value })} />
        </div>
        <div className="form-group">
          <label><FormattedMessage id="order_number" /></label>
          <Input disabled={true} className="form-control" type="text" id="order_number" name="order_number" value={this.state.order_number} onChange={e => this.setState({ order_number: e.target.value })} />
        </div>
        <div className="form-group">
