...
import DatatableDetailCmpt from '../shared/datatable-detail-follow-up-done';
...
class Assignments extends Components {
  ...
  getOrders = () => {
    const assignmentOrderFollowUps = this.props.assignmentOrderFollowUps
    .filter(followUp => followUp.id_current_step === null);
    const assignmentOrders = this.props.assignmentOrders;
    const newData = assignmentOrderFollowUps.map(fu => {
  ...
  render() {
    ...
    const data = this.getOrders().filter(order => order.id_timeline === 'portability');
    ...
    return (
      <div className="container">
        <Datatable data={data} >
          ...
          <Datatable.DataColumn field="product" transform={this.transform.label} title={this.format({ id: 'label' })} size={4} sortable={true} search={this.search.label} />
