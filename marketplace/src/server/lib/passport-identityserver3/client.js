...
Client.prototype.getTokens = function(req, options, callback) {
    ...
    getAccessToken(req.session, config, params, options, callback);
};
...
Client.prototype.authorizationUrl = function(req, state, options) {
    var config = this.config,
        params = extend({}, {
            state: state,
            response_type: 'code',
            ...
            redirect_uri: this.callbackUrl(req),
            scope: this.scope(),
        }, config.authorize_params, options);
...
function getAccessToken(session, config, params, options, callback) {
    extend(params, {
        client_id: config.client_id,
