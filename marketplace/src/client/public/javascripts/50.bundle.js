...
/***/ function(module, exports) {
	...
	exports.flattenExcludedPermissions = flattenExcludedPermissions;
	exports.hasCircularDependencies = hasCircularDependencies;
	exports.validatePermission = validatePermission;
	exports.getExcludedPermissions = getExcludedPermissions;
	exports.validatePermissionList = validatePermissionList;
	/**
	...
	function flattenPermissions(permissions) {
	  return permissions.reduce(function (p, n) {
	    return p.concat(p.indexOf(n) === -1 ? n : [], flattenPermissions(n.includes ? n.includes.filter(function (i) {
	 ...
	 */
	function flattenExcludedPermissions(permissions) {
	  return permissions.reduce(function (prev, next) {
	    return prev.concat(next.excludes || []);
	  }, []);
	}
	 ...
	 */
	function hasCircularDependencies(permissions) {
	  try {
	 ...
	 */
	function validatePermission(permissions, permission) {
	  var currentExcludedPermissions = flattenExcludedPermissions(permissions);
	  var permissionsToBeAdded = flattenPermissions([permission]);
	  var newExcludedPermissions = flattenExcludedPermissions(permissionsToBeAdded);
	  var validation = { valid: true, errors: [] };
	  permissionsToBeAdded.forEach(function (p) {
	    var errorObject = { permission: p, conflictsWith: [] };
	    p.excludes.forEach(function (e) {
	      if (permissions.map(function (perm) {
	        return perm.id;
	      }).indexOf(e.id) !== -1) {
	...
	function getExcludedPermissions(permissions, permission) {
	  var currentExcludedPermissions = flattenExcludedPermissions(permissions);
	  var permissionsToBeAdded = flattenPermissions([permission]);
	  var newExcludedPermissions = flattenExcludedPermissions(permissionsToBeAdded);
	  return permissionsToBeAdded.reduce(function (p, n) {
	    return currentExcludedPermissions.indexOf(n) !== -1 ? p.concat(n) : p;
	  }, []);
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _permissions3 = __webpack_require__(1005);
	...
	var permissionsToArray = function permissionsToArray(permissions) {
	  var permissionArray = [];
	...
	var Permissions = function (_Components) {
	  _inherits(Permissions, _Components);
	
	  function Permissions(props) {
	    _classCallCheck(this, Permissions);
	
	    var _this = _possibleConstructorReturn(this, (Permissions.__proto__ || Object.getPrototypeOf(Permissions)).call(this, props));
	
	    _this.onChange = function (_ref) {
	      var value = _ref.target.value;
	      return _this.navigate('/admin/users/acl/edit/' + _this.props.params.id + '/' + value);
	    };
	
	    _this.updatePermission = function (permission) {
	      return function (_ref2) {
	        var target = _ref2.target;
	
	        target.disabled = true;
	        var activePermissions = permissionsToArray(_this.state.permissions).filter(function (op) {
	          return op.enabled;
	        });
	        var addPermission = permission.enabled === false;
	        if (addPermission) {
	          var permissionValidation = (0, _permissions3.validatePermission)(activePermissions, permission);
	          if (!permissionValidation.valid && !permission.enabled) {
	            var permissionConflicts = permissionValidation.errors.map(function (pv) {
	              return pv.permission.id + '\nconflicts with:\n' + pv.conflictsWith.map(function (cw) {
	                return cw.id;
	              }).join(', ');
	            }).join('\n\n');
	            var alertMsg = 'PERMISSION CONFLICTS\n\n  The following permissions would be excluded by the permission you are trying to add:\n\n  ' + permissionConflicts;
	            alert(alertMsg);
	            target.disabled = false;
	            return;
	          }
	        }
	        var newActivePermissions = addPermission ? activePermissions.concat((0, _permissions3.flattenPermissions)([permission])) : [].concat(activePermissions.slice(0, activePermissions.indexOf(permission)), activePermissions.slice(activePermissions.indexOf(permission) + 1));
	        new _actions2.default(_this.props.tenant).BackOffice.Acl.updateRolePermissions({
	          role: _this.state.role.id,
	          permissions: newActivePermissions.map(function (p) {
	            return p.id;
	          })
	        }).then(function (action) {
	          _this.props.dispatch(action);
	          _this.props.notification.add({ message: 'Save Successful', level: 'success' });
	          target.disabled = false;
	        }).catch(function (err) {
	          console.log(err);
	          ...
	          _this.props.notification.add({ message: 'An error occurred', level: 'error' });
	          target.disabled = false;
	        });
	      };
	    };
	
	    _this.updateName = function (_ref3) {
	      var target = _ref3.target;
	
	      new _actions2.default(_this.props.tenant).BackOffice.Acl.update({
	        role: _this.state.role.id,
	        name: _this.state.role.name
	      }).then(function (action) {
	        _this.props.dispatch(action);
	    ...
	    _this.getPermissionUrl = function (permissionId, roleId) {
	      console.log('permissionId', permissionId);
	      console.log('roleId', roleId);
	      if (permissionId !== _permissions2.default.ACCESS_BI360_FOLLOW_UPS.id) {
	        return 'No match';
	      }
	      switch (roleId) {
	        case 22:
	    ...
	    _this.state = _this.fetchRoles(props);
	    return _this;
	  }
	  ...
	  _createClass(Permissions, [{
	    ...
	    key: 'fetchRoles',
	    value: function fetchRoles(props) {
	      var _this2 = this;
	
	      var role = props.roles.toJS().find(function (r) {
	        return r.id === parseInt(_this2.props.params.id, 10);
	      });
	      return {
	        role: role,
	        permissions: Object.keys(_permissions2.default).map(function (permission) {
	          return _extends({}, _permissions2.default[permission], {
	            enabled: !!role.permissions.includes(permission)
	          });
	        }).reduce(function (prev, permission) {
	          var category = permission.category.toLowerCase();
	          if (!prev[category]) {
	            prev[category] = [];
	          }
	    ...
	    value: function render() {
	      ...
	      var activePermissions = permissionsToArray(this.state.permissions).filter(function (p) {
	        return p.enabled;
	      });
	      var excludedPermissions = (0, _permissions3.flattenExcludedPermissions)(activePermissions);
	      var roles = this.state.permissions[this.props.params.category.toLowerCase()].map(function (permission) {
	        var urlText = _this3.getPermissionUrl(permission.id, _this3.state.role.id);
	        return _react2.default.createElement(
	          _Row2.default,
	          { className: 'rows-alt' },
	          _react2.default.createElement(
	            _Col2.default,
	            { xs: 3 },
	            _react2.default.createElement(_reactIntl.FormattedMessage, { id: permission.id })
	          ),
	          _react2.default.createElement(
	            _Col2.default,
	            { xs: 8 },
	            permission.hasUrl && _react2.default.createElement('input', { disabled: true, style: { width: '100%' }, type: 'text', value: urlText })
	          ),
	          _react2.default.createElement(
	            _Col2.default,
	            { xs: 1 },
	            _react2.default.createElement('input', { id: 'row' + permission.id, onChange: _this3.updatePermission(permission), type: 'checkbox', checked: permission.enabled, disabled: excludedPermissions.map(function (p) {
	                return p.id;
	      ...
	      return _react2.default.createElement(
	        ...
	        _react2.default.createElement(
	          _Col2.default,
	          { md: 9 },
	          _react2.default.createElement(_Input2.default, { type: 'text', value: this.state.role.name,
	            onChange: function onChange(_ref4) {
	              var value = _ref4.target.value;
	              _this3.setState({ role: _extends({}, _this3.state.role, { name: value }) });
	            },
	            onBlur: this.updateName
	          }),
	          _react2.default.createElement(
	            'div',
	            { style: { border: '1px solid #f5f5f5', borderRadius: 3, padding: '0 15px' } },
	            roles
	          )
	        ),
	        _react2.default.createElement(
	          ...
	          _react2.default.createElement(
	            ...
	            _react2.default.createElement(
	              ...
	              Object.keys(_permissions.CATEGORY).map(function (category) {
	                return _react2.default.createElement(
	                  'option',
	                  { key: _permissions.CATEGORY[category], value: _permissions.CATEGORY[category].toLowerCase() },
	                  _permissions.CATEGORY[category] + ' - ' + _this3.state.permissions[_permissions.CATEGORY[category].toLowerCase()].filter(function (p) {
	                    return p.enabled;
	                  }).length + '/' + _this3.state.permissions[_permissions.CATEGORY[category].toLowerCase()].length
	                );
	              })
	            )
	          )
