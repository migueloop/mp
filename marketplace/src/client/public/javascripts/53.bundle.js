...
/***/ function(module, exports, __webpack_require__) {
	...
	var Content = (_temp = _class = function (_Components) {
	  _inherits(Content, _Components);
	  ...
	  _createClass(Content, [{
	    ...
	    value: function componentDidMount() {
	      Promise.all(Content.getActions(this.props.tenant, this.props.params).map(this.props.dispatch));
	    }
	  }, {
	    key: '_getMenuItems',
	    value: function _getMenuItems() {
	      ...
	      if (permissions.isIntersected([_constants.PERMISSIONS.EDIT_PRODUCT.id, _constants.PERMISSIONS.EDIT_PRODUCT_OWN.id])) {
	        menuitems.push({ url: '/admin/content/products', title: this.format({ id: 'products' }) });
	      }
	      if (permissions.isIntersected([_constants.PERMISSIONS.EDIT_BUNDLE.id, _constants.PERMISSIONS.EDIT_BUNDLE_OWN.id])) {
	        menuitems.push({ url: '/admin/content/bundles', title: this.format({ id: 'bundles' }) });
	      }
	      if (permissions.isIntersected([_constants.PERMISSIONS.OFFER_PRODUCT.id])) {
	        menuitems.push({ url: '/admin/offers', title: this.format({ id: 'offers' }) });
	      }
	      if (permissions.isIntersected([_constants.PERMISSIONS.EDIT_STOCK.id, _constants.PERMISSIONS.EDIT_STOCK_OWN.id])) {
	    ...
	    key: 'numPendingProducts',
	    get: function get() {
	      return this.props.products.filter(function (a) {
	        return a.state === 'pending';
	      }).length || 0;
	    }
	  }, {
	    key: 'numPendingBundles',
	    get: function get() {
	      return this.props.bundles.filter(function (a) {
	        return a.state === 'pending';
	      }).length || 0;
	    }
	  }, {
	    key: 'numPendingAssignmentOrders',
	    get: function get() {
	      var _this3 = this;
	
	      return this.props.assignmentOrders.filter(function (order) {
	        return !!order.stepName && order.usersWithAccessToCurrentStep.indexOf(_this3.props.user.id) !== -1;
	      }).length;
	    }
	  }], [{
	    key: 'getActions',
	    value: function getActions(tenant) {
	      ...
	      return [actions.BackOffice.Products.fetchAll(), actions.BackOffice.Bundles.fetchAll(), actions.BackOffice.AssignmentsOrder.fetchAll()];
	    }
	  }]);
	...
	var _components = { default: connector(Content) };
