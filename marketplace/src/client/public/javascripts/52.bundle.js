	...
	var Users = function (_Components) {
	  ...
	  _createClass(Users, [{
	    ...
	    value: function _getMenuItems() {
	      ...
	      var permissions = this.props.user.toJS().permissions;
	      var menuitems = [];
	
	      if (_underscore2.default.contains(permissions, _constants.PERMISSIONS.READ_USERS.id) && _underscore2.default.contains(permissions, _constants.PERMISSIONS.READ_USERS_DETAILS.id)) {
	        menuitems.push({ url: '/admin/users/summary', title: formattedMessage({ id: 'view_users' }) });
	      }
	
	      if (_underscore2.default.contains(permissions, _constants.PERMISSIONS.CREATE_USER.id)) {
	        menuitems.push({ url: '/admin/users/create', title: formattedMessage({ id: 'user_management' }), id: 'create-users' });
	      }
	
	      if (_underscore2.default.contains(permissions, _constants.PERMISSIONS.EDIT_ROLES_PERMISSION.id)) {
	    ...
	    value: function render() {
	      ...
	      var menu = menuitems.map(function (data) {
	        return _react2.default.createElement(
	          _reactRouter.Link,
	          { key: data.id, to: data.url, activeClassName: 'active', className: 'generalMenuItem', id: data.id },
	          data.title
	        );
	      });
