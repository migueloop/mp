...
/***/ function(module, exports, __webpack_require__) {
	...
	var Domain = (_temp = _class = function (_Components) {
	  _inherits(Domain, _Components);
	  ...
	  _createClass(Domain, null, [{
	    ...
	    value: function getActions(tenant) {
	      ...
	      var domainId = Domain.getId(params);
	      var actions = new _flux.Actions(tenant);
	      return [actions.FrontOffice.Domains.fetchOne(domainId), actions.FrontOffice.Domains.fetchProducts(domainId)];
	    }
	  }, {
	    ...
	    value: function getClientActions(tenant) {
	      ...
	      var actions = new _flux.Actions(tenant);
	      return [actions.FrontOffice.Domains.fetchProducts(Domain.getId(params)), actions.FrontOffice.Domains.fetchBundles(Domain.getId(params))];
	    }
	  }]);
	  ...
	  _createClass(Domain, [{
	    ...
	    value: function render() {
	      ...
	      var domain = this.props.domain.toJS();
	      var highlightProducts = domain.products.filter(function (product) {
	        var currentDomain = product.domains.find(function (productDomain) {
	          return productDomain.id === domain.id;
	        });
	        return currentDomain.product_corner.highlight_product > 0;
	      });
	      return _react2.default.createElement(
	        'section',
	        { className: 'mp-corner thematic ' + (domain.name ? 'thematic-' + domain.name.toLowerCase() : '') },
	        _react2.default.createElement(
	          'div',
	          { className: 'container' },
	          ...
	          _react2.default.createElement(Products, { show: this.state.active === 'products', keywords: domain.keywords, products: domain.products }),
	          _react2.default.createElement(Bundles, { show: this.state.active === 'bundles', keywords: domain.keywords, bundles: domain.bundles })
	        )
	      );
	    }
	  }]);
	  ...
	  this.load = function (props) {
	    _this2.setState({
	      ...
	      Promise.all(Domain.getActions(props.tenant, props.params).concat(Domain.getClientActions(_this2.props.tenant, _this2.props.params)).map(_this2.props.dispatch)).then(function () {
